...
1. On mount, `AuthProvider` calls `GET /api/me`. If the user has a valid session
   cookie, `user` is populated. Otherwise `user` is `null`.
2. `AuthGate` wraps the protected route group. If `user` is `null` (and not still
...
- Sends credentials (`credentials: 'include'`) so the session cookie works
